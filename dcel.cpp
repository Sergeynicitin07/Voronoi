...
Vertex we_should_find_centre(const Vertex& a, const Vertex& b, const Vertex& c) {
    ...
    double ux = (sa * (b.y - c.y) + sb * (c.y - a.y) + sc * (a.y - b.y)) / D;
    double uy = (sa * (c.x - b.x) + sb * (a.x - c.x) + sc * (b.x - a.x)) / D;
...
bool point_in_circle(const Vertex& a, const Vertex& b, const Vertex& c, const Vertex& p) {
    long double adx = (long double)a.x - p.x;
    long double ady = (long double)a.y - p.y;
    long double bdx = (long double)b.x - p.x;
    long double bdy = (long double)b.y - p.y;
    long double cdx = (long double)c.x - p.x;
    long double cdy = (long double)c.y - p.y;

    long double ab = adx * adx + ady * ady;
    long double bb = bdx * bdx + bdy * bdy;
    long double cb = cdx * cdx + cdy * cdy;

    long double det = ab * (bdx * cdy - bdy * cdx)
                      - bb * (adx * cdy - ady * cdx)
                      + cb * (adx * bdy - ady * bdx);
